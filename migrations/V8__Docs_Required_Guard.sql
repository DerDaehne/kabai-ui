-- V8__Docs_Required_Guard.sql
-- Opt-in documentation requirement per ticket (kbai-docs #328):
-- a ticket flagged docs_required cannot move to 'done' without at least one
-- linked knowledge-base note (note_ticket_links) — enforced in the DB,
-- mirroring the existing acceptance-criteria check.

ALTER TABLE tickets ADD COLUMN IF NOT EXISTS docs_required BOOLEAN NOT NULL DEFAULT FALSE;

-- Extend the workflow trigger (V4 body + docs check in the done branch).
        ...
        IF NEW.docs_required THEN
            ...
            IF linked_notes_count = 0 THEN
                RAISE EXCEPTION
                    ...
                    'kb.ai_docs_link_ticket or unset docs_required with a justification.',
                    NEW.id;
            END IF;
        END IF;
    END IF;
