...
{#if visible}
	<div
		class="rounded-lg p-4 max-w-sm animate-in fade-in slide-in-from-bottom-2"
		style="background: var(--card-bg); box-shadow: var(--elevation-2);"
	...
	>
		<div class="flex items-start gap-3">
			...
			<div class="flex-shrink-0">
				...
				{:else if type === 'error'}
					<XCircle class="w-5 h-5" style="color: var(--danger);" />
				{:else if type === 'warning'}
					<AlertTriangle class="w-5 h-5" style="color: var(--warning);" />
				{:else}
					<Info class="w-5 h-5" style="color: var(--primary);" />
				{/if}
			</div>
			...
			<div class="flex-1 min-w-0">
				{#if title}
					<p class="font-medium truncate" style="color: var(--text);">{title}</p>
				{/if}
				<p class="text-sm truncate-2" style="color: var(--text-muted);">{message}</p>
			</div>

			<!-- Close Button -->
			<button
				onclick={() => visible = false}
				class="btn-ghost flex-shrink-0 !p-1 rounded-md"
